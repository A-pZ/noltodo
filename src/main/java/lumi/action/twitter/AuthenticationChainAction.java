...
public class AuthenticationChainAction extends LumiActionSupport {
	...
	public String calback() throws Exception {
		...
		val twitter = (Twitter) getSession().get(SessionKeys.TWITTER4J_INSTANCE.name());
		if ( twitter == null ) {
		...
		val reqToken = (RequestToken)getSession().get(SessionKeys.REQUEST_TOKEN.name());
		if ( reqToken == null ) {
		...
		// AccessTokenの取得
		val accessToken = twitter.getOAuthAccessToken(reqToken, oauth_verifier);
		...
		val requestKeyFlag = getSession().get(SessionKeys.TWITTER_ACCOUNT_CHAIN_REQUEST_KEY.name());
		...
		// Twitter認証連携を登録する
		val isChain = service.chainSpringSecurity(twitter);
		if ( !isChain ) {
		...
		val isAuth = service.startAuthentication();
		if ( !isAuth ) {
